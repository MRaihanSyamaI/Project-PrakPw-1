...
<body>
  ...
  // Query untuk mencari lagu berdasarkan judul atau artis
  $sql = "SELECT * FROM `songs` WHERE title LIKE '$sc'  OR artist LIKE '$sc';";
  $stmt = $conn->prepare($sql);
  // $stmt->bind_param('ss', $search, $search);
  $stmt->execute();
  $hasil_sc = $stmt->get_result();
  ?>
  ...
  <main id="search">
    ...
    <!-- Hasil Pencarian Lagu -->
    <?php if ($sc): ?>
      <!-- // Pastikan query dijalankan dan menghasilkan hasil -->
      ...
      <table class="search-results">
        ...
        <?php while ($hasil = $hasil_sc->fetch_assoc()): ?>
          <?php
          // Mengambil cover image lagu (BLOB)
          $song_cover = $hasil['cover_image']; // Gambar BLOB
          ...
          <tr>
            ...
            <td>
              <div
                ...
                onclick="playSong(<?= $hasil['id']; ?>, '<?= addslashes($hasil['title']); ?>', '<?= addslashes($hasil['artist']); ?>', '<?= base64_encode($hasil['cover_image']); ?>')">
